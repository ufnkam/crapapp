...
impl<'a> WindowsBundler<'a> {
    ...
    fn write_setup_project(
        ...
        self.write_setup_build_input(platform, files, shortcuts, None, setup_source_dir)?;
    ...
    fn build_uninstaller(
        ...
        let status = Command::new("cargo")
            ...
            .arg("--release")
            .arg("--target-dir")
            .arg(setup_source_dir.join("target"))
            .arg("--target")
            .arg(target)
            .arg("--bin")
            .arg("uninstall")
...
fn setup_cargo_toml(installer: WindowsInstaller) -> Result<String> {
    ...
    template
        .render(context! {
            libcrapapp_dependency => dependencies.runtime,
            libcrapapp_build_dependency => dependencies.build,
        })
