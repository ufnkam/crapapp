...
use crate::gui::settings_screen::Settings;
...
impl Installer {
    fn new(config: UiConfig) -> (Self, Task<Message>) {
        ...
        let installed = config
            .installer_config()
            .as_ref()
            .is_some_and(crate::registry::registry_install_exists);
    ...
    fn next_screen(&mut self, current_screen: Screen) -> Task<Message> {
        ...
        if let Screen::Process(process) = self.screen {
            self.process_state = crate::gui::process_screen::State::running(process);
            return crate::gui::process_screen::start(
                process,
